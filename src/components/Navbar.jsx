...
const Navbar = () => {
   ...
   return (
        <nav className="px-6 py-5 flex items-center justify-between max-w-[1320px] mx-auto relative">
            ...
            <div
                className={`fixed top-0 right-0 w-full h-screen bg-black z-20 flex flex-col items-center justify-center space-y-6 transform transition-transform duration-500  ${menuOpen ? 'translate-x-0' : 'translate-x-full'
                    } md:hidden`}>
                {navLinks.map((link,index) => (
                    <NavLink key={index}  to={link.path} className={({ isActive }) =>
                            `font-bold text-lg ${isActive ? 'text-red-500' : 'text-white'}`}
